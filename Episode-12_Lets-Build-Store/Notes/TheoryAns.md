# Episode 11 - Playing with Data: Summary

## What We Did in Episode 11

## 1. Higher Order Components (HOCs)
**What**: Functions that take a component and return an enhanced component  
**Problem Solved**: Adding "Promoted" labels to restaurant cards without duplicating code  
**Implementation**: Created `withPromotedLabel` HOC that wraps `RestaurantCard`  
**Result**: Reusable promotion feature that can be applied to any component
...
## 2. Controlled Components & Lifting State Up
**What**: Managing component state from parent instead of individual components  
**Problem Solved**: Restaurant menu accordion where only one category opens at a time  
**Implementation**: Moved accordion state from `RestaurantCategory` to `RestaurantMenu` parent  
**Result**: Coordinated behavior - opening one accordion closes others automatically

```javascript
// Parent controls which accordion is open
...
**What**: Global state management to avoid prop drilling  
**Problem Solved**: User data needed in Header, Body, and RestaurantCard components  
**Implementation**: Created `UserContext` with Provider wrapping the app  
**Result**: Any component can access/modify user data without passing props through intermediate components
